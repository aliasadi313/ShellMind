...
def handle_queryv(args):
    ...
    try:
        ...
        if not hasattr(config_manager, 'DEFAULT_CONFIG'):
            print("Error: Configuration system not properly initialized")
            return
        ...
        if not config_manager.get("api_key"):
            try:
                ai_interaction_check = AIInteraction()
                if not ai_interaction_check.api_key and not isinstance(ai_interaction_check.openai):
                    print("Error: AI API key is not configured. Please set it using:")
                    print("  shellmind config set api_key YOUR_API_KEY")
                    return
            except ConfigError as ce:
                print(f"Configuration Error: {ce}")
                return
            except AIInteractionError as aie:
                print(f"AI Interaction Error: {aie}")
                return
    ...
    except AIInteractionError as e:
        print(f"ShellMind AI Error: {e}")
    except CommandExecutionError as e:
        print(f"ShellMind Execution Error: {e}")
    except ConfigError as e:
        print(f"ShellMind Configuration Error: {e}")
    except ShellMindError as e:
        print(f"ShellMind Error: {e}")
    except Exception as e:
        print(f"An unexpected ShellMind error occurred: {e}")
